...
export default function EditCampaignDialog({ open, onOpenChange, campaign }) {
  ...
  useEffect(() => {
    if (campaign && open) {
      setFormData({
        ...
        durationMonths: campaign.duration_months || 1,
      ...
      setFlyerUrl(campaign.flyer_url || "");
  ...
  const updateCampaignMutation = useMutation({
    mutationFn: (data) => base44.entities.AdvertiseApplication.update(campaign.id, data),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto" style={{ background: '#0F2744', border: '1px solid rgba(255, 255, 255, 0.18)' }}>
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3 text-2xl" style={{ color: '#E5EDFF' }}>
            <div className="w-10 h-10 rounded-xl flex items-center justify-center" style={{ background: '#6366F1' }}>
              <Edit className="w-5 h-5" style={{ color: '#fff' }} />
            </div>
            Edit Campaign - {campaign.business_name}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 mt-4">
          ...
          {/* Campaign Details */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold" style={{ color: '#E5EDFF' }}>Campaign Details</h3>

            <div>
              <Label htmlFor="objectives" style={{ color: '#B6C4E0' }}>Campaign Objectives *</Label>
              <Textarea
                id="objectives"
          ...
          {/* Creative Upload */}
          <div className="space-y-4">
            <h3 className="text-lg font-semibold" style={{ color: '#E5EDFF' }}>Ad Creative</h3>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: '#E5EDFF' }}>
                        {uploadedFile ? uploadedFile.name : 'Current creative'}
                      </p>
                      {uploadedFile && (
